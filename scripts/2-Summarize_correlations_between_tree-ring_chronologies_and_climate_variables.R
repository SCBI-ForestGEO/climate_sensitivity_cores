...
for( c in climate.data.types) {
  ...
  all.dc.corr <- read.csv(paste0("results/Going_back_at_earliest_common_year/tables/monthly_correlation/correlation_with_", c, "_climate_data.csv"))
  ...
  write.csv(rbind(summary_of_correlations, summary_of_significance, summary_of_significant_correlations),
        file = paste0("results/Going_back_at_earliest_common_year/tables/monthly_correlation/SUMMARY_Correlation_with_", c, "_climate_data.csv"), row.names = F)
