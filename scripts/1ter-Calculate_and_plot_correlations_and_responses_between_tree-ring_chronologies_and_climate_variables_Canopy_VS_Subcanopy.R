...
write.csv(all_sss, file = "results/canopy_vs_subcanopy/SSS_as_a_function_of_the_number_of_trees_in_sample.csv", row.names = F)
...
for(c_or_sc in c("canopy", "subcanopy")) {
  ...
  if(save.plots) tiff(paste0("results/canopy_vs_subcanopy/SSS_as_a_function_of_the_number_of_trees_in_sample_", toupper(c_or_sc), ".tiff"), res = 150, width = 169, height = 169, units = "mm", pointsize = 10)
...
for(c in climate.data.types) {
  ...
  for(type.start in type.of.start.date) {
    ...
    dir.create(paste0("results/canopy_vs_subcanopy/", type.start), showWarnings = F)
    dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/figures"), showWarnings = F)
    dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/tables"), showWarnings = F)
    ...
    for(method.to.run in methods.to.run) {
      ...
      dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/tables/monthly_", method.to.run ), showWarnings = F)
      ...
      if(method.to.run %in% c("correlation", "response")) {  
        ...
        if(save.result.table) write.csv(all.dcc.output, file = paste0("results/canopy_vs_subcanopy/", type.start, "/tables/monthly_", method.to.run, "/", method.to.run, ifelse(grepl("corr", method.to.run), "_with_", "_to_"), c, "_climate_data.csv"), row.names = F)
      }
      
      if(method.to.run %in% c("moving_correlation") & type.start %in% "1901_2009") {
        if(save.result.table) save(all.dcc.output, file = paste0("results/canopy_vs_subcanopy/", type.start, "/tables/monthly_moving_correlation/moving_correlation_with_", c, "_climate_data.Rdata"))
      }
      ...
      if(method.to.run %in% c("correlation")) {
        for(v in names(clim)[-c(1,2)]) {
          ...
          if(save.plots)  {
            dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_", method.to.run), showWarnings = F)
            dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_", method.to.run, "/", c), showWarnings = F)
            tiff(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_", method.to.run, "/", c, "/", v, ".tif"), res = 150, width = 169, height = 169, units = "mm", pointsize = 10)
          }
      ...
      if(method.to.run %in% c("moving_correlation") & type.start %in% "1901_2009") {
        ...
        for(f in filenames) {
          ...
          for(v in names(clim)[-c(1,2)]) {
            ...
            if(save.plots)  {
              dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_moving_correlation"), showWarnings = F)
              dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_moving_correlation/by_species_and_by_month"), showWarnings = F)
              dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_moving_correlation/by_species_and_by_month/", c), showWarnings = F)
              tiff(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_moving_correlation/by_species_and_by_month/", c, "/", f, "_", v, ".tif"), res = 150, width = 169, height = 169, units = "mm", pointsize = 10)
            }
        ...
        for(v in names(clim)[-c(1,2)]) {
          ...
          for(mth in switch(v,frs = c("apr", "may"), c("apr", "may", "jun", "jul"))) {
            ...
            if(save.plots) {
              dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_moving_correlation/by_curr_season_month_all_sp_together"), showWarnings = F)
              dir.create(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_moving_correlation/by_curr_season_month_all_sp_together/", c), showWarnings = F)
              tiff(paste0("results/canopy_vs_subcanopy/", type.start, "/figures/monthly_moving_correlation/by_curr_season_month_all_sp_together/", c, "/", v, "_", mth, ".tif"), res = 150, width = 169, height = 169, units = "mm", pointsize = 10)
            }
...
dir.create("results/canopy_vs_subcanopy/climate/", showWarnings = F)

write.csv(mean_and_std_of_clim, file = "results/canopy_vs_subcanopy/climate/mean_and_std_of_climate_variables.csv", row.names = F)
