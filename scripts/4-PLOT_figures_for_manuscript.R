...
if(save.plots)  {
  tiff(paste0("results/figures_for_manuscript/Figure_3.tif"), res = 300, width = 169, height = 120, units = "mm", pointsize = 10)
}
...
# Figure 4 (comparison of ANPP in the 3 time periods) ####

## Define how to run it regarding the starting year ####
type.of.start.date <- c("1920_1949", "1950_1979", "1980_2009")


## plot ####

if(save.plots)  {
  tiff(paste0("results/figures_for_manuscript/Figure_4.tif"), res = 300, width = 180, height = 70, units = "mm", pointsize = 10)
}


nf <- layout(mat = matrix(c(1,2,3,4), ncol = 4, byrow = T), widths = c(1,1,1,0.4)) # nf <- layout(mat = matrix(c(1,2,3), ncol = 3, byrow = T), widths = c(1,1,0.4))
...
for(type.start in type.of.start.date) {
  ...
  # title ####
  if(plot.nb %in% c(1,2,3)) title(main, line = 5, outer = F)
  ...
  par(xpd= NA)
  if(plot.nb %in% c(1,2,3)) {
    lines(x = 1:9, y = rep(12.7, 9), col = "grey", lwd = 2)
    lines(x = 10:17, y = rep(12.7, 8), lwd = 2)
    text(x = 5, y = 12.7, labels = "previous year", col = "grey", pos = 3)
    text(x = 14, y = 12.7, labels = "current year", pos = 3)
  } else {
    lines(x = 1:9, y = rep(12.7, 9), col = "grey", lwd = 2)
    lines(x = 10:17, y = rep(12.7, 8), lwd = 2)
  }
...
text(x = 9, y = start.unit + (i * leg.unit) + c(5, 3), labels = c(expression(bold("ANPP Response")), expression(bold(Mg ~ ha^-1 ~ yr^-1 ~" - %"))))
