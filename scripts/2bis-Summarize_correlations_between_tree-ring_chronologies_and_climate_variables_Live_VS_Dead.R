######################################################
# Purpose: Summarize correlations (or response functions) between tree-ring chronologies and climate variables , looking separately at trees cored while alive and trees cored while dead
...
for(type.start in type.of.start.date) {
  ...
  for( c in climate.data.types) {
    ...
    for(season in c("all_seasons", "curr_growing_season_only")) {
      ...
      all.dc.corr <- read.csv(paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/correlation_with_", c, "_climate_data.csv"))
  ...
  if(save.result.table) write.csv(All_species_combined_both_season_subsets_all_climates, file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_Correlation_with_all_climates_both_seasons_species_combined.csv"), row.names = F)
...
for(type.start in type.of.start.date) {
  ...
  if(save.result.table) {
    write.csv(n_positive_corr, file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_n_positive_correlation.csv"), row.names = F)
    write.csv(n_significant_corr, file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_n_significant_correlation.csv"), row.names = F)
    write.csv(mean_corr, file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_mean_correlation.csv"), row.names = F)
    write.csv(min_corr, file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_min_correlation.csv"), row.names = F)
    write.csv(max_corr, file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_max_correlation.csv"), row.names = F)
...
for(type.start in c("1901_2009", "1980_2009")) {
  assign(paste("n_positive_corr", type.start, sep = "_"), read.csv(file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_n_positive_correlation.csv")))
  assign(paste("n_significant_corr", type.start, sep = "_"), read.csv(file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_n_significant_correlation.csv")))
  assign(paste("mean_corr", type.start, sep = "_"), read.csv(file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_mean_correlation.csv")))
  assign(paste("min_corr", type.start, sep = "_"), read.csv(file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_min_correlation.csv")))
  assign(paste("max_corr", type.start, sep = "_"), read.csv(file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_max_correlation.csv")))
}
...
# dir.create("results/live_vs_dead/tables_for_manuscript/", showWarnings = F)
# print(doc, target = "results/live_vs_dead/tables_for_manuscript/Supplementary_tables_Pearson_correlation_summary_per_variable.docx")
# 
