...
for(type.start in type.of.start.date) {
  ...
  for( c in climate.data.types) {
    ...
    for(season in c("all_seasons", "curr_growing_season_only")) {
      ...
      # if(save.result.table) write.csv(summary_to_save, file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_Correlation_with_", c, "_", season, ".csv"), row.names = F)
    ...
    # if(save.result.table) write.csv(All_species_combined_both_season_subsets, file = paste0("results/live_vs_dead/", type.start, "/tables/monthly_correlation/SUMMARY_Correlation_with_", c, "_both_seasons_species_combined.csv"), row.names = F)
